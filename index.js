...
function findElementGreaterThan(arr, num) {
  ...
  let foundElement = arr.find((foundElement) => foundElement > num);
  if (foundElement === arr) {
    return num;
  } else {
    return foundElement;
...
function getPositiveArrayValues(arr) {
  ...
  Array.from(iterator);
  let positiveArrayValues = arr.filter((iterator) => iterator > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  arr.splice(startIdx);
  let doubledArray = arr.map((startIdx) => startIdx * 2);
  return doubledArray;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let filteredSquaredArray = mapArr.filter((element) => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let firstValue = 0;
  let sum = lastElement.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    firstValue
  );
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((accumulator, currentValue) => {
...
function capitalizeWords(arr) {
  ...
  let capitalizedArray = arr.map((element) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  let lastIndex = arr.findLastIndex((num) => num < 0);
  return lastIndex;
}
